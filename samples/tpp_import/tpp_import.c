...
static iree_status_t iree_samples_tpp_import_provider_resolve(
  ...
  if (iree_string_view_equal(symbol_name,
                             IREE_SV("xsmm_brgemm_dispatch"))) {
    *out_fn_ptr = iree_xsmm_brgemm_dispatch;
  ...
  if (iree_string_view_equal(symbol_name, IREE_SV("xsmm_binary_dispatch"))) {
    *out_fn_ptr = iree_xsmm_binary_dispatch;
    return iree_ok_status();
  }

  // Xsmm invoke API.
  if (iree_string_view_equal(symbol_name, IREE_SV("xsmm_brgemm_invoke"))) {
    *out_fn_ptr = iree_xsmm_brgemm_invoke;
    return iree_ok_status();
  }
  ...
  if (iree_string_view_equal(symbol_name, IREE_SV("xsmm_binary_invoke"))) {
    *out_fn_ptr = iree_xsmm_binary_invoke;
    return iree_ok_status();
  }
